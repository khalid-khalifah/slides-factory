...
def test_strict_template_rejects_invalid_nested_props():
    template = get_template("strict")
    with pytest.raises(ValidationError):
        template.validate_data({"headline": {"text": 123}})


def test_simple_template_round_trip(tmp_path: Path):
    ...
    document.add_slide(prs, "simple", data)
...
def test_add_slide_with_frame_requires_brand(tmp_path: Path):
    output = tmp_path / "plain.pptx"
    document.create_document(output)
    prs = document.open_document(output)
    with pytest.raises(ValueError, match="--frame"):
        document.add_slide(prs, "simple", {"headline": {"text": "X"}}, frame="plain")


def test_add_slide_applies_frame_when_branded(tmp_path: Path, minimal_brand_yaml: Path):
    output = tmp_path / "framed.pptx"
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "Framed"}}, frame="plain")
    document.save_document(prs, output)
...
def test_add_slide_uses_brand_default_frame(tmp_path: Path, minimal_brand_yaml: Path):
    output = tmp_path / "default.pptx"
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "T"}})
    document.save_document(prs, output)
...
def test_add_slide_uses_template_default_frame(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    prs = document.open_document(output)
    document.add_slide(prs, "paired", {"title": "T", "headline": {"text": "T"}})
    document.save_document(prs, output)
...
def test_edit_slide_preserves_stored_frame(tmp_path: Path, minimal_brand_yaml: Path):
    output = tmp_path / "stored.pptx"
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "One"}}, frame="alt")
    document.edit_slide(prs, 0, {"headline": {"text": "Two"}})
    document.save_document(prs, output)
...
def test_edit_slide_template_change_uses_new_template_default(
    tmp_path: Path, minimal_brand_yaml: Path
):
    ...
    document.add_slide(prs, "simple", {"headline": {"text": "One"}}, frame="alt")
    document.edit_slide(
        prs, 0, {"title": "Paired", "headline": {"text": "Paired"}}, template_id="paired"
    )
    document.save_document(prs, output)
...
def test_get_frame_unknown_raises():
    with pytest.raises(KeyError, match="Unknown frame"):
        get_frame("not-a-frame")


def test_remove_slide_out_of_range(tmp_path: Path):
    output = tmp_path / "deck.pptx"
    document.create_document(output)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "One"}})
    with pytest.raises(IndexError):
        document.remove_slide(prs, 5)


def test_edit_slide_requires_template_metadata(tmp_path: Path):
    output = tmp_path / "deck.pptx"
    document.create_document(output)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "One"}})
...
def test_add_slide_locks_frame_shapes_when_enabled(tmp_path: Path, minimal_brand_yaml: Path):
    import zipfile
    ...
    document.create_document(output, brand=brand_path)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"headline": {"text": "Locked"}}, frame="chrome")
    document.save_document(prs, output)
