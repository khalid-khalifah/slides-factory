...
def test_strict_template_rejects_incomplete_json():
    template = get_template("strict")
    with pytest.raises(ValidationError):
        template.validate_data({"title": "Only title"})
...
def test_simple_template_round_trip(tmp_path: Path):
    ...
    assert info["data"]["title"] == "Hello"
    assert info["data"]["body"] == "World"
...
def test_add_slide_with_frame_requires_brand(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="--frame"):
        document.add_slide(prs, "simple", {"title": "X"}, frame="plain")
...
def test_add_slide_applies_frame_when_branded(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"title": "Framed"}, frame="plain")
    document.save_document(prs, output)
...
def test_add_slide_uses_brand_default_frame(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"title": "T"})
    document.save_document(prs, output)
...
def test_add_slide_uses_template_default_frame(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    prs = document.open_document(output)
    document.add_slide(prs, "paired", {"title": "T"})
    document.save_document(prs, output)
...
def test_edit_slide_preserves_stored_frame(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"title": "One"}, frame="alt")
    document.edit_slide(prs, 0, {"title": "Two"})
    document.save_document(prs, output)
...
def test_edit_slide_template_change_uses_new_template_default(
    ...
    document.add_slide(prs, "simple", {"title": "One"}, frame="alt")
    document.edit_slide(
        prs, 0, {"title": "Paired"}, template_id="paired"
    )
    document.save_document(prs, output)
...
def test_remove_slide_out_of_range(tmp_path: Path):
    ...
    document.create_document(output)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"title": "One"})
...
def test_add_slide_locks_frame_shapes_when_enabled(tmp_path: Path, minimal_brand_yaml: Path):
    ...
    document.create_document(output, brand=brand_path)
    prs = document.open_document(output)
    document.add_slide(prs, "simple", {"title": "Locked"}, frame="chrome")
    document.save_document(prs, output)
