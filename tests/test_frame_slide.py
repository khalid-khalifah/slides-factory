...
def test_add_frame_slide_round_trip(tmp_path: Path, minimal_brand_yaml: Path, app):
    output = tmp_path / "cover.pptx"
    document.create_document(output, brand=minimal_brand_yaml)
    prs = document.open_document(output)
    document.add_frame_slide(prs, "cover", {}, app=app)
...
def test_layout_blocked_on_no_playground_frame(tmp_path: Path, minimal_brand_yaml: Path, app):
...
def test_playground_box_raises_when_layout_disallowed(app):
    frame = get_frame(app, "cover")
